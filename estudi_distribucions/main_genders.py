...
def main():
    ...
    # Agrupar per gènere i calcular mitjana
    genders = ['female', 'male']
    cer_by_gender = {}
    for gender in genders:
        # Seleccionar splits que acabin amb gender
        vals = [cer for spl, cer in cer_per_split.items() if spl.endswith(gender)]
        if vals:
            cer_by_gender[gender] = float(np.mean(vals)) if vals else None
            print(f"CER mitjà per {gender}: {cer_by_gender[gender]:.4f}")
            wandb.log({f"CER/{gender}": cer_by_gender[gender]})
        else:
            print(f"No hi ha dades pel gènere: {gender}")

    # 3) Generar gràfic
    if cer_by_gender:
        plt.figure(figsize=(6, 4))
        genders_amb_dades = list(cer_by_gender.keys())
        valors_cer = list(cer_by_gender.values())
        
        bars = plt.bar(genders_amb_dades, valors_cer, color=['#1f77b4', '#ff7f0e'])
        plt.ylabel("CER mitjà")
        plt.title("Comparativa de CER per gènere")
        
        # Afegir valors als bars
        for bar, valor in zip(bars, valors_cer):
            plt.text(
        ...
        out_fig = os.path.join(out_dir, "cer_by_gender.png")
        
        plt.savefig(out_fig, dpi=300, bbox_inches='tight')
        wandb.log({"cer_by_gender_plot": wandb.Image(out_fig)})
        print(f"\nGràfic de gènere desat a {out_fig}")
        plt.show()
    else:
