...
BASE_DATA_DIR = "/mnt/c/Users/HP/Downloads/rosany_datasets"
...
def main():
    ...
    wandb.init(
        ...
        name="cer_by_dialecte",
        config={
            "batch_size": 32,
            "model_checkpoint": "model_speech_to_text_finetunedv1.pt",
            "splits": SPLITS,
            "max_words_test": 200,
            "resample_rate": 16000
        }
    ...
    cache_dir  = os.path.join(SCRIPT_DIR, "output_filesv1")
    os.makedirs(cache_dir, exist_ok=True)
    cache_file = os.path.join(cache_dir, "cer_per_splitv1.json")
    ...
    for dialecte in dialectes:
        # Seleccionar splits que comencen amb el nom del dialecte
        vals = [cer for spl, cer in cer_per_split.items() if spl.startswith(dialecte)]
        if vals:
            cer_by_dialecte[dialecte] = float(np.mean(vals))
            print(f"CER mitjà per {dialecte}: {cer_by_dialecte[dialecte]:.4f} (n={len(vals)} splits)")
            wandb.log({f"CER/{dialecte}": cer_by_dialecte[dialecte]})
        else:
            print(f"No hi ha dades per al dialecte: {dialecte}")

    # 3) Generar
    if cer_by_dialecte:
        # 1) Map de codi amb etiqueta en català
        label_map = {
            'balearic': 'Balear',
            'central': 'Central',
            'northern': 'Nord',
        ...
        # 2) Creem i ordenem la llista de tuples (codi, cer, label)
        data = [
            (dialecte, cer, label_map.get(dialecte, dialecte))
            for dialecte, cer in cer_by_dialecte.items()
        ]
        data.sort(key=lambda x: x[1])  # orden ascendent pel CER

        # 3) Desempaquetem en llistes paral·leles
        codes_ord, cers_ord, labels_ord = zip(*data)

        # 4) Colors segons l’índex
        cmap   = plt.get_cmap('tab10')
        colors = [cmap(i) for i in range(len(cers_ord))]

        # 5) Dibuixem el barh amb les etiquetes en català i ordenat
        ...
        plt.title("Comparativa de CER per dialecte català")

        # 6) Afegim els valors al final de cada barra
        ...
        out_dir = os.path.join(SCRIPT_DIR, "output_filesv1")
        os.makedirs(out_dir, exist_ok=True)
        out_fig = os.path.join(out_dir, "cer_by_dialectev1.png")

        plt.savefig(out_fig, dpi=300, bbox_inches='tight')
        wandb.log({"cer_by_dialecte_plot": wandb.Image(out_fig)})
